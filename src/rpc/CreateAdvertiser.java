...
/**
 * CREATE TABLE advertiser ( advertiser_id int NOT NULL AUTO_INCREMENT, name
 * varchar(255) NOT NULL, budget int, PRIMARY KEY (advertiser_id) );
 */
public class CreateAdvertiser extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			RpcHelper.writeJsonObject(response, new JSONObject().put("advertiser_id", advertiser_id));
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			conn.close();
